...
public class BaseGun : MonoBehaviour
{
    ...
    virtual protected void Update()
    {
        ...
        if (curWaitTime*WeaponHanger.ATKSpeedMul >= GetColorBuffedCooldown()) //무기 발사 가능여부 체크
        {
            CanShot = true;
        }
        else
    ...
    protected virtual float GetColorBuffedDamage()
    {
        float dmg = bulletDamage;
        switch (weaponColor)
        {
            case ColorType.Red:   dmg *= 1.15f; break;
            case ColorType.Blue:  dmg *= 0.9f; break;
            case ColorType.Green: dmg *= 0.85f; break;
        }
        return dmg;
    }

    protected virtual float GetColorBuffedCooldown()
    {
        float cd = 1f / shootSpeed;
        if (weaponColor == ColorType.Red) cd *= 1.1f;
        return cd;
    }

    protected virtual float GetColorBuffedBulletSpeed()
    {
        float spd = bulletSpeed;
        switch (weaponColor)
        {
            case ColorType.Blue: spd *= 1.3f; break;
            case ColorType.Green: spd *= 0.85f; break;
        }
        return spd;
    }
